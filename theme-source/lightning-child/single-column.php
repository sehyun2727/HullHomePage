<?php
/**
 * Single column post template (/column/{post-name}/)
 * Picked up via WP standard template hierarchy (single-column.php),
 * which Lightning's template_hierarchy_redirect() also checks first
 * regardless of G2/G3 mode. Copied from Lightning G2's _g2/singular.php
 * (the shared template every other post type still uses) to drop the
 * breadcrumb without touching that shared file.
 */
?>
...
<?php // No breadcrumb on single column posts — see archive-column.php, taxonomy-column_category.php, taxonomy-column_tag.php. ?>
