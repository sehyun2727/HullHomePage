<?php
/**
 * Works category taxonomy archive template (/works/category/{slug}/)
 * Mirrored from taxonomy-column_category.php — column → works throughout.
 */

$show_tags = true;

$works_category_term = get_queried_object();
?>
...
<div class="row">
	...
	<div class="<?php lightning_the_class_name( 'mainSection' ); ?>" id="main" role="main">
		...
		<?php works_render_category_tabs( $works_category_term instanceof WP_Term ? $works_category_term->slug : '' ); ?>

		<div class="works-search-bar">
			<h2 class="works-search-bar__title"><?php echo esc_html( $works_category_term instanceof WP_Term ? $works_category_term->name : '事例一覧' ); ?></h2>
			...
			<div class="works-archive__grid card-grid">
				...
				$search_extra_query = new WP_Query( array(
					...
					'tax_query'      => array( array(
						...
						'field'    => 'term_id',
						'terms'    => $works_category_term instanceof WP_Term ? $works_category_term->term_id : 0,
					) ),
				) );
