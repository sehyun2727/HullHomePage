<?php
/**
 * Column post type archive template (/column/)
 * Picked up via WP standard template hierarchy (archive-column.php),
 * which Lightning's template_hierarchy_redirect() also checks first
 * regardless of G2/G3 mode. Header/breadcrumb block copied from
 * Lightning G2's _g2/index.php to keep the same site chrome.
 */
...
<?php // No breadcrumb on this archive; category/tag archives and single columns still render theirs. ?>
...
<div class="row">
	...
	<div class="<?php lightning_the_class_name( 'mainSection' ); ?>" id="main" role="main">
		...
		<?php
		// column_render_search_card() and column_render_category_tabs() live in
		// functions.php — shared with taxonomy-column_category.php.
		column_render_category_tabs( '' );
		?>

		<div class="column-search-bar">
			<h2 class="column-search-bar__title">コラム一覧</h2>
		...
		<?php if ( have_posts() ) : ?>
			...
			<div class="column-archive__grid card-grid">
				...
				?>
				<?php
				// Every other published column post, pre-rendered but hidden
				// (see .column-card--extra in style.css) so search can reach it.
